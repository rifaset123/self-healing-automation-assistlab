...
export const khsLocators = {
  ...
  khsHeader: (page: Page) => page.getByText("📤 KHS/Transkrip"),
  // menambah KHS
  ...
  uploadedKhsFileName: (page: Page, filename: string) => page.getByText(filename.replace('.pdf', '')),
  ...
  verifyFilteredData: (page: Page, search: string) => page.locator("tbody tr").filter({ hasText: search }),
